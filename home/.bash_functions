...
function archive() {
  for path in "$@"; do
    ...
    cd "$dir"
    tar -ac --exclude=node_modules --exclude=__pycache__ --exclude=dist --exclude=build --exclude=.next --exclude=.venv --exclude=.cache --exclude=venv --exclude-vcs -f "$dest.tar.xz" "$dest"
    cd -
  done
}
