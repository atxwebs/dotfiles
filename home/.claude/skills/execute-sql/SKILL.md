...
- Use `scripts/sql.sh "SELECT ..."` from this skill dir. Runs from project dir (or any subdir); finds .claude/mcp.json upward.
- If arg ends with `.sql`, uses `psql -f` (e.g. for migration files).
- Or manually: `DATABASE_URI=$(grep -oP '"DATABASE_URI"\s*:\s*"\K[^"]*' .claude/mcp.json | head -n1)` then `psql "$DATABASE_URI" -c "SELECT ..."`
