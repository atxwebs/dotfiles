...
def parse_args():
    ...
    # Parse flags
    download_mmproj = False
    if '--mmproj' in args:
        download_mmproj = True
        args.remove('--mmproj')
    
    if len(args) < 1:
        print("Usage: hf.py [--mmproj] <owner/project> [quant] [custom_name]")
        print("  --mmproj    Download mmproj file for vision models")
        print("Example: hf.py unsloth/Qwen3.5-9B-GGUF IQ3_XXS")
        print("Example: hf.py --mmproj bartowski/Qwen_Qwen3-VL-8B-Instruct-GGUF Q4_K_M")
        sys.exit(1)
    ...
    return repo, quant, custom_name, download_mmproj
...
def main():
    repo, quant, custom_name, download_mmproj_flag = parse_args()
    ...
    # Download mmproj only if --mmproj flag was provided
    mmproj_basename = None
    if download_mmproj_flag:
    ...
    add_to_preset(model_base, mmproj_basename)
    
    print(f"\nDone: {dest}")
    if download_mmproj_flag and mmproj_basename:
