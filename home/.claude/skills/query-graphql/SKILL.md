...
If MCP unavailable, use curl as fallback. Extract endpoint and headers with: `cat .claude/mcp.json | grep -A 2 -E '127\.0\.0\.1:[0-9]+/_/gql'`
...
This file is in `~/.claude/skills/query-graphql/` - take all paths as relative to it. Related: [run-server](./run-server) - for running the server.